...
String mb="File block2";
String mby="cloud server2";
String sta="new";
try
	{
		con=databasecon2.getconnection();
		ps = con.prepareStatement("select uid,name,fid,fname,fsize,AES_DECRYPT(b2,'key'),date,status from filestore where fid ='"+fid+"'");
		rs = ps.executeQuery();
		while(rs.next())
		{      
		        uid=rs.getString(1);
	   ...
	   if(b1.equals(data1))
	   {
		 response.sendRedirect("cloud2_all_files.jsp");
	   ...
	   else
	   {
	        try
			   ...
			   ps1.setString(8,strDateNew1);
				...
				response.sendRedirect("cloud2_all_files.jsp?message=fail");
		     }
               catch(Exception e2)
	            {
